        ...
        <div className="min-h-screen bg-gray-50 p-3 sm:p-4 pb-24">
            <div className="max-w-md mx-auto space-y-4">
                ...
                {rows.map((p: any, idx: number) => {
                    ...
                    const color = st.includes('paid') ? 'green' : st.includes('pending') ? 'gold' : 'default';
                    return (
                        <Card
                            key={p?.id ?? idx}
                        ...
                        >
                            <div className="flex items-center justify-between">
                                <div className="text-sm font-semibold text-gray-900">ID: {p?.id ?? '-'}</div>
                                <Tag className="m-0" color={color as any}>{st}</Tag>
                            </div>
                            <div className="mt-2 text-xs text-gray-600">Amount: {p?.amount ?? '-'}</div>
                            <div className="mt-1 text-xs text-gray-500">Created: {p?.createdAt ? String(p.createdAt) : '-'}</div>
                        </Card>
                    );
                })}
